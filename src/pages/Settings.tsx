    ...
    const handleLogin = async () => {
        if ((window as any).Capacitor?.isNativePlatform) {
            try {
                ...
                // The plugin reads clientId from capacitor.config on Android/iOS
                const userContext = await GoogleAuth.signIn();
                const token = userContext.authentication.accessToken;
                
                setAccessToken(token);
                localStorage.setItem('gdriveToken', token);
                setUserEmail(userContext.email);
                localStorage.setItem('gdriveEmail', userContext.email);
                fetchBackupInfo();
            } catch (err: any) {
                console.error('Native login failed:', err);
                setSyncStatus('error');
                setSyncMessage('App login failed: ' + (err.message || 'Unknown error'));
            }
        } else {
            loginWeb();
        }
